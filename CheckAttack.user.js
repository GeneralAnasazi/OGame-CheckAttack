    ...
    static getSerializeType(node) {
        ...
        if (node.hasChildNodes && node.firstChild && node.firstChild.nodeName !== null && node.firstChild.nodeName !== "#text") {
            ...
            if (node.hasAttributes)
                result = 1; //node with attributes 
            else
                result = 0; //only properties
        }
        return result;
    }
    ...
    static getApiHeadObj(responseHeader) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (keyName !== "") {
                switch (keyName) {
                    case "content-length":
                        result[keyName] = parseInt(values[1]);
                        break;
                    case "date":
                    case "expires":
                    case "last_modified":
...
class CombatReport extends Report {
    ...
    /**
     * calculates the losses of an Combat Report
     * @param {Array} units
     * @param {Array} [repairedDefense]
     * @returns {Ressources} the lost ressources
     */
    ...
    /**
     * 
     * @param {SpyReportList} spyReportList 
     * @param {CombatReportList} reportList 
     */
